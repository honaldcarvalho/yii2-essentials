...
class AppendModel extends \yii\bootstrap5\Widget
{
    ...
    public function run()
    {
        ...
        $button = $this->new_button && AuthorizationController::verAuthorization(get_class(Yii::$app->controller), 'model-get') ? Html::a('<i class="fas fa-plus-square"></i> Novo', "javascript:modal_{$this->attactModel}.show();{$this->newCallBack}", ['class' => 'btn btn-success', 'id' => "btn-show-{$this->uniqueId}"]) : '';
        ...
        $head = <<< HTML
            <div class="card mt-3" id="list-{$this->uniqueId}">
