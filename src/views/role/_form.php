...
$controllerBaseName = function (?string $fqcn): ?string {
  if (!$fqcn || !class_exists($fqcn)) return null;
  $short = (new \ReflectionClass($fqcn))->getShortName();
  return preg_replace('/Controller$/', '', $short);
};
...
if (!$model->isNewRecord) {
  ...
  $controllerShort = $controllerBaseName($controllerFQCN);; // crie esse método se necessário, baseado no `path:controller`
  $availableActions = [];
      ...
      <div class="col-md-5">
        <select name="from[]" id="multiselect" class="form-control" size="8" multiple="multiple">
          <?php foreach ($fromActions as $action): ?>
            <option value="<?= $action ?>"><?= ($controllerShort ? $controllerShort . '\\' : '') . $action ?></option>
          <?php endforeach; ?>
        </select>
      </div>
      <div class="col-md-2">
